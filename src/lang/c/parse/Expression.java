...
class ExpressionSub extends CParseRule {
	...
	public void semanticCheck(CParseContext pcx) throws FatalErrorException {
		...
		if (left != null) {
			...
			pcx.fatalError(sub.toExplainString() + "左辺がありません");
		}
		if (right != null) {
			right.semanticCheck(pcx);
			rt = right.getCType().getType();	// -の右辺の型
			...
			pcx.fatalError(sub.toExplainString() + "右辺がありません");
		}
		int nt = s[lt][rt];						// 規則による型計算
		if (nt == CType.T_err) {
	...
	public void codeGen(CParseContext pcx) throws FatalErrorException {
		...
		if (left != null && right != null) {
			right.codeGen(pcx);
			left.codeGen(pcx);
			o.println("\tMOV\t-(R6), R0\t; ExpressionSub: ２数を取り出して、引き、積む<" + sub.toExplainString() + ">");
